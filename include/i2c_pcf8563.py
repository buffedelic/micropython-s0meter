...
from include.timezone import TZONE
...
class PCF8563(object):
    def __init__(self, i2c, i2c_addr, zone=0, dht=True):
        ...
        if self.i2c_addr in self.i2c.scan():
            print('RTC: PCF8563 found at address: 0x%x ' % (self.i2c_addr))
        else:
    ...
    def datetime(self, datetime=None):
        ...
        if datetime == None:
            ...
            #print('RTC: {:0>2d}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}'.format(yy, MM, dd, hh, mm, ss))
            # wday in esp8266 0 == Monday, 6 == Sunday
            return yy, MM, dd, hh, mm, ss, wday, 0
        elif datetime != None:
            ...
            self.i2c.writeto_mem(self.i2c_addr, _SECONDS,
                                 self._tobytes(self._dec2bcd(ss)))
            if mm < 0 or mm > 59:  # Записываем новое значение минут
                raise ValueError('RTC: Minutes is out of range [0,59].')
            self.i2c.writeto_mem(self.i2c_addr, _MINUTES,
                                 self._tobytes(self._dec2bcd(mm)))
            if hh < 0 or hh > 23:  # Записываем новое значение часов
                raise ValueError('RTC: Hours is out of range [0,23].')
            self.i2c.writeto_mem(self.i2c_addr, _HOURS, self._tobytes(
                self._dec2bcd(hh)))  # Sets to 24hr mode
            if mday < 1 or mday > 31:  # Записываем новое значение дней
                raise ValueError('RTC: Date is out of range [1,31].')
            self.i2c.writeto_mem(self.i2c_addr, _DATE, self._tobytes(
                self._dec2bcd(mday)))  # Day of month
            if wday < 0 or wday > 6:  # Записываем новое значение дней недели
                raise ValueError('RTC: Day is out of range [0,6].')
            self.i2c.writeto_mem(self.i2c_addr, _WDAY,
                                 self._tobytes(self._dec2bcd(wday)))
            if MM < 1 or MM > 12:  # Записываем новое значение месяцев
                raise ValueError('RTC: Month is out of range [1,12].')
            self.i2c.writeto_mem(self.i2c_addr, _MONTH,
                                 self._tobytes(self._dec2bcd(MM)))
            if yy < 0 or yy > 99:  # Записываем новое значение лет
                raise ValueError('RTC: Years is out of range [0,99].')
            self.i2c.writeto_mem(self.i2c_addr, _YEAR,
                                 self._tobytes(self._dec2bcd(yy)))
            # Cчитываем записанное новое значение времени с PCF8563
            (yy, MM, mday, hh, mm, ss, wday, yday) = self.datetime()
            print('RTC: New Time: {:0>2d}-{:0>2d}-{:0>2d} {:0>2d}:{:0>2d}:{:0>2d}'.format(
                yy, MM, mday, hh, mm, ss))  # Выводим новое время PCF8563
    ...
    def settime(self, source='dht'):
        ...
        if source == 'esp':  # Устанавливаем время с часов ESP8266
            ...
            # Время с NTP без учета летнего или зимнего времени в секундах начиная с 2000г
            ntp = self.tzone.getntp()
            # Корректируем время по временным зонам
            z = self.tzone.adj_tzone(localtime(ntp)) if self.dht else 0
        ...
        if self.block and rtc[1] == 10:
            if rtc[3] == 2:  # Если 2 часа, блокировка не снимается
                pass
            # Если 3 часа и меньше 2 минут, бликировка не снимается
            elif rtc[3] == 3 and rtc[4] <= 2:
                pass
